...
The `risk` dataset contains financial and demographic attributes that help assess a customer's likelihood of being classified as either a *good risk* or *bad risk*. This dataset, included in the **liver** package, consists of `r nrow(risk)` observations and `r ncol(risk)` variables. It provides a structured way to analyze customer characteristics and predict financial risk levels.  

The dataset includes `r ncol(risk) - 1` predictors and a binary target variable, `risk`, which distinguishes between customers who are more or less likely to default. The key variables are:  
...
For example, if the second row has a probability of `r round(prob_naive_bayes[2,2], 3)` for "`bad risk`," it indicates that the second customer in the test set is predicted to belong to the "`bad risk`" category with a probability of `r round(prob_naive_bayes[2,2], 3)`.  
...
The values in the confusion matrix quantify the model’s classification accuracy and error rates at a cutoff of 0.5. Specifically, the model correctly predicts "`r conf.mat(prob_naive_bayes, test_labels, cutoff = 0.5, reference = "good risk")[1, 1]` + `r conf.mat(prob_naive_bayes, test_labels, cutoff = 0.5, reference = "good risk")[2, 2]`" cases and misclassifies "`r conf.mat(prob_naive_bayes, test_labels, cutoff = 0.5, reference = "good risk")[1, 2]` + `r conf.mat(prob_naive_bayes, test_labels, cutoff = 0.5, reference = "good risk")[2, 1]`" cases.  
