...
The estimated proportion of churners in the dataset is `r round(prop.table(table(churn$churn))[1], 2)`, serving as our best guess for the proportion of churners in the population.
:::
...
If the computed interval is [`r round(ci_lower, 2)`, `r round(ci_upper, 2)`], we can say with 95% confidence that the true average number of service calls for churners falls within this range.
...
The output includes the *p*-value, test statistic, degrees of freedom, and confidence interval for the population mean. Since the *p*-value = `r round(t_test$p.value, 4)` is less than the significance level (\(\alpha = 0.05\)), we reject the null hypothesis (\(H_0\)). This would indicate that there is sufficient evidence, at the 5% significance level, to conclude that the true average number of customer service calls differs from 2.

The test also provides a 95% confidence interval, [`r round(t_test$conf.int, 2)`], which represents the range of plausible values for the true population mean. Since 2 is outside this interval, we have further evidence that the true average number of service calls is different from the assumed value. Additionally, the sample mean, `r round(t_test$estimate, 2)`, is reported as the best estimate of the population mean.
...
The *p*-value indicates the probability of obtaining the observed sample proportion if the assumed population proportion were true. Since the *p*-value = `r round(prop_test$p.value, 4)`, and it is greater than \(\alpha = 0.05\), we do not reject the null hypothesis. This means there is insufficient evidence to conclude that the churn rate in the population differs from 15%. In this case, we report:  
...
The test also provides a *95% confidence interval*, [`r round(prop_test$conf.int, 2)`], which represents the plausible range for the true population proportion (\(\pi\)). If 0.15 lies within this interval, it supports failing to reject \(H_0\). If 0.15 is outside the interval, it strengthens the evidence against \(H_0\).  

Additionally, the test reports the *sample proportion*, `r round(prop_test$estimate, 2)`, which is the observed proportion of churners in the dataset. This value serves as an estimate of the true population proportion.
:::
...
The *p*-value = `r round(t_test_calls$p.value, 4)`. Since this value is less than the significance level (\(\alpha = 0.05\)), we reject the null hypothesis, concluding that the mean number of international calls differs significantly between churners and non-churners. The *95% confidence interval* = [`r round(t_test_calls$conf.int, 2)`] provides a range of plausible values for the true difference in means. If the interval does not include zero, we have statistical evidence that the two groups differ significantly in their average number of international calls. The test output also provides the *sample means*:  
...
The output provides the *p*-value, confidence interval, and sample proportions. Since the *p*-value (`r round(z_test$p.value, 4)`) is smaller than the significance level (\(\alpha = 0.05\)), we reject the null hypothesis. This result suggests that the proportion of customers with a Voice Mail Plan differs significantly between churners and non-churners. 

The test also provides a *95% confidence interval* = [`r round(z_test$conf.int, 4)`] for the difference in proportions. Since this interval does not include zero, it reinforces the conclusion that the difference is statistically significant. Additionally, the sample proportions—`r round(z_test$estimate[1], 4)` for churners and `r round(z_test$estimate[2], 4)` for non-churners—provide insight into the magnitude of this difference.
...
The output includes the *p*-value, Chi-square test statistic, degrees of freedom, and expected frequencies under \(H_0\). If the *p*-value = `r chisq_test$p.value` is smaller than \(\alpha = 0.05\), we reject the null hypothesis, concluding that marital status and deposit purchases are not independent. This means that at least one marital group differs significantly from the others in deposit purchase rates.
...
- **p-value**: If the *p*-value = `r round(cor_test$p.value, 4)` is smaller than the significance level (\(\alpha = 0.05\)), we reject \(H_0\), confirming that the correlation is statistically significant.
- **Correlation Coefficient**: The correlation coefficient (\(r = `r round(cor_test$estimate, 2)`\)) quantifies the strength and direction of the relationship. A value close to 1 indicates a strong positive correlation, while a value near 0 suggests no linear association.
- **Confidence Interval**: The 95% confidence interval [`r round(cor_test$conf.int, 2)`] provides a plausible range for the true population correlation (\(\rho\)). If this interval does not include 0, it further supports rejecting \(H_0\) and confirms a meaningful association.

The correlation coefficient of `r round(cor_test$estimate, 2)` suggests a strong positive relationship between *carat* and *price*, meaning that larger diamonds tend to be more expensive. The small *p*-value confirms that this pattern is unlikely due to random variation, while the confidence interval provides an estimate of how precisely we can measure this correlation.
