...
```{r knn-image, echo = FALSE, out.width = "75%", fig.align = 'center', fig.cap = "A two-dimensional toy dataset with two classes (Class A and Class B) and a new data point (dark star), illustrating the k-Nearest Neighbors algorithm with k = 3 and k = 6."}
...
```{r scatter-plot-ex-drug, echo = FALSE, out.width = "95%", fig.align = 'center', fig.cap = "Scatter plot of Age vs. Sodium/Potassium Ratio for 200 patients, with drug type indicated by color and shape."}
library(ggplot2)
...
```{r scatter-plot-ex-drug-2, echo = FALSE, out.width = "95%", fig.align = 'center', fig.cap = "Scatter plot of Age vs. Sodium/Potassium Ratio for 200 patients, with drug type indicated by color and shape. The three new patients are represented by large orange circles."}
...
p + geom_point(data = test_data, aes(x = Age, y = Ratio), colour = "orange", size = 15, shape = 1) +
...
```{r scatter-plot-ex-drug-3, echo = FALSE, fig.align = 'default', fig.show = "hold", out.width = "33%", fig.cap = "Zoom-in plots for the three new patients and their nearest neighbors. The left plot is for Patient 1, the middle plot is for Patient 2, and the right plot is for Patient 3."}
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Patient 1") +
    geom_point(aes(x = x, y = y), colour = "orange", size = 120, shape = 1) +
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Patient 2") +
    geom_point(aes(x = x, y = y), colour = "orange", size = 120, shape = 1) +
    geom_point(aes(x = x, y = y), colour = "orange", size = 8, shape = 4) 
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Patient 3") +
    geom_point(aes(x = x, y = y), colour = "orange", size = 120, shape = 1) +
    geom_point(aes(x = x, y = y), colour = "orange", size = 8, shape = 4)
```
...
```{r kNN-plot, echo = FALSE, out.width = "85%", fig.align = 'center', fig.cap = "Accuracy of the k-Nearest Neighbors algorithm for different values of k in the range from 1 to 30."}
...
From the plot, we observe that kNN accuracy fluctuates as \( k \) increases. The highest accuracy is achieved when \( k = `r which.max(kNN.plot_output)` \), where the algorithm balances sensitivity to local patterns with robustness to noise. At this value, kNN delivers an accuracy of `r max(kNN.plot_output)` and an error rate of `r 1 - max(kNN.plot_output)`.
...
```{r ex-proper-scaling, echo = FALSE, out.width = "50%", fig.align = 'center', fig.cap = "Visualization illustrating the difference between proper scaling and improper scaling. The left panel shows the original data without scaling. The middle panel shows the results of proper scaling. The right panel shows the results of improper scaling."}
# To plot the data without scaling
ggplot(data = train_data, aes(x = Age, y = Ratio)) +
  geom_point(aes(color = Type, shape = Type)) + 
  labs(title = "Data without Scaling", 
       x = "Age", y = "Sodium/Potassium Ratio") + 
  guides(color = guide_legend(title = "Drug Type"), 
         shape = guide_legend(title = "Drug Type")) +
...
ggplot(data = train_scaled, aes(x = Age, y = Ratio)) +
  geom_point(aes(color = Type, shape = Type)) + 
  labs(title = "Data with Proper Scaling", 
       x = "Age", y = "Sodium/Potassium Ratio") + 
  guides(color = guide_legend(title = "Drug Type"), 
         shape = guide_legend(title = "Drug Type")) +
...
ggplot(data = train_scaled_wrongly, aes(x = Age, y = Ratio)) +
  geom_point(aes(color = Type, shape = Type)) + 
  labs(title = "Data with Improper Scaling", 
       x = "Age", y = "Sodium/Potassium Ratio") + 
  guides(color = guide_legend(title = "Drug Type"), 
         shape = guide_legend(title = "Drug Type")) +
...
The dataset is a *data.frame* in **R** with `r nrow(churn)` observations and `r ncol(churn) - 1` predictor variables. The target variable, *churn*, indicates whether a customer has churned (`yes`) or not (`no`).
...
From the confusion matrix, we see that the model correctly classified `r conf_max_knn_churn[1, 1] + conf_max_knn_churn[2, 2]` instances, while `r conf_max_knn_churn[1, 2] + conf_max_knn_churn[2, 1]` instances were misclassified. This summary helps assess model performance and identify areas for improvement.
