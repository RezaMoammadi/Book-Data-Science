...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Pationt 1") +
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Pationt 2") +
    geom_point(aes(x = x, y = y), colour = "orange", size = 120, shape = 1) +
    geom_point(aes(x = x, y = y), colour = "orange", size = 8, shape = 4) 
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
  labs(title = "Zoom-in Plot for New Pationt 3") +
    geom_point(aes(x = x, y = y), colour = "orange", size = 120, shape = 1) +
    geom_point(aes(x = x, y = y), colour = "orange", size = 8, shape = 4)
```
...
From the plot, we observe that the accuracy of the kNN algorithm fluctuates as \( k \) increases. In this example, the highest accuracy is achieved when \( k = `r which.max(kNN.plot_output)` \). At this value, the kNN algorithm balances sensitivity to local patterns with robustness to noise, delivering an accuracy of `r max(kNN.plot_output)` and an error rate of `r 1 - max(kNN.plot_output)`.
...
```{r ex-proper-scaling, echo = FALSE, out.width = "50%", fig.align = 'center', fig.cap = "Visuliaztion to liiustrate the difference of proper scaling and improper scaling. The leff panel shows the original data without scaling. The middle panel shows the results of proper scaling. The right panel shows the results of improper scaling."}
...
The confusion matrix summarizes the number of correct and incorrect predictions. In this case, the model achieves "`r conf_max_knn_churn[1, 1]` + `r conf_max_knn_churn[2, 2]`" correct predictions and "`r conf_max_knn_churn[1, 2]` + `r conf_max_knn_churn[2, 1]`" incorrect predictions. This provides insights into the model’s performance and highlights areas for improvement.
