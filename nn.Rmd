...
```{r net-brain, echo = FALSE, out.width = "75%", fig.align = 'center', fig.cap = "Visualization of a biological neuron, which processes input signals through dendrites and sends outputs through the axon."}
knitr::include_graphics("images/net_brain.png")
```

```{r net-1, echo = FALSE, out.width = "90%", fig.align = 'center', fig.cap = "Illustration of an artificial neuron, designed to emulate the structure and function of a biological neuron in a simplified way."}
knitr::include_graphics("images/net_1.png")
```
...
```{r net-reg, echo = FALSE, out.width = "40%", fig.align = 'center', fig.cap = "A graphical representation of a regression model: input features and predictions are shown as nodes, with the coefficients represented as connections between the nodes."}
knitr::include_graphics("images/net_reg.png")
```
...
```{r net-large, echo = FALSE, out.width = "80%", fig.align = 'center', fig.cap = "Visualization of a multilayer neural network model with two hidden layers."}
knitr::include_graphics("images/net_large.png")
```
...
```{r active-fun-unit, echo = FALSE, out.width = "50%", fig.align = 'center', fig.cap = "Visualization of the threshold activation function (unit step)."}
...
```{r active-fun-sigmoid, echo = FALSE, out.width = "50%", fig.align = 'center', fig.cap = "Visualization of the sigmoid activation function."}
...
```{r active-fun-comparison, echo = FALSE, out.width = "80%", fig.align = 'center', fig.cap = "Comparison of common activation functions: Sigmoid, tanh, and Gaussian."}
x_values <- seq(-5, 5, by = 0.001)
sigmoid <- 1 / (1 + exp(-x_values))
tanh <- tanh(x_values)
gaussian <- exp(-x_values^2)

data <- data.frame(x = x_values, Sigmoid = sigmoid, Tanh = tanh, Gaussian = gaussian)

ggplot(data) +
  geom_line(aes(x = x, y = Sigmoid, color = "Sigmoid"), size = 1.2) +
  geom_line(aes(x = x, y = Tanh, color = "Tanh"), size = 1.2) +
  geom_line(aes(x = x, y = Gaussian, color = "Gaussian"), size = 1.2) +
  scale_color_manual(name = "Activation Functions", 
                     values = c("Sigmoid" = "blue", "Tanh" = "green", "Gaussian" = "red")) +
...
The dataset contains `r nrow(bank)` observations and `r ncol(bank)` variables. It includes `r ncol(bank) - 1` predictors and one target variable (`deposit`). Below is a summary of the key variables:  
