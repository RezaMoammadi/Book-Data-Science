...
ggplot_1 = filter(gapminder, year %in% c(1962, 2015) & !is.na(group) & 
                !is.na(fertility) & !is.na(life_expectancy)) |>
    ggplot(aes(x = fertility, y = life_expectancy, col = group, size = population / 10 ^ 6)) +
    geom_point(alpha = 0.8) +
    guides(size = "none") +
    coord_cartesian(ylim = c(30, 85)) + 
    labs(x = 'Total Fertility Rate (Births per Woman)', 
...
if(knitr::is_html_output()) {
    ...
    ggplot_1 + geom_text(aes(x = 6, y = 80, label = year), cex = 17, color = "#eddee4") +
    facet_grid(. ~ year) + 
