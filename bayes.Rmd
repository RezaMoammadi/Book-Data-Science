...
The `risk` dataset is a `data.frame` with `r ncol(risk)` variables and `r nrow(risk)` observations. It contains `r ncol(risk) - 1` predictors and a target variable `risk`, which is a binary factor with two levels: "`good risk`" and "`bad risk`." The predictors in this dataset are:  
...
The values in the confusion matrix reveal the model’s accuracy and types of errors at a cutoff of 0.5. For example, the classifier makes "`r conf.mat(prob_naive_bayes, actual_test, cutoff = 0.5, reference = "good risk")[1, 1]` + `r conf.mat(prob_naive_bayes, actual_test, cutoff = 0.5, reference = "good risk")[2, 2]`" correct predictions and "`r conf.mat(prob_naive_bayes, actual_test, cutoff = 0.5, reference = "good risk")[1, 2]` + `r conf.mat(prob_naive_bayes, actual_test, cutoff = 0.5, reference = "good risk")[2, 1]`" incorrect predictions.
...
The AUC value, `r round(auc(roc_naive_bayes), 3)`, quantifies the model’s ability to rank positive instances higher than negative ones. A value closer to 1 indicates excellent performance, while a value of 0.5 represents random guessing.  
